...
中文：
后续工作应优先分析 v1.5/v1.6 在 ISIC 2017 test 上的域偏移和 student 退化原因，并在新增独立验证证据后再设计下一轮训练。继续扩大模型或外部数据规模之前，需要先证明 OOF 改进能够迁移到 independent test。

English:
Future work should first diagnose the v1.5/v1.6 domain shift and student degradation on the ISIC 2017 test split. Another large training round should require evidence that OOF gains transfer to independent test before increasing model or external-data scale again.
